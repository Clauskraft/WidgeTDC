...
- **Broken path alias:** The Vite alias maps `@` to `<repo>/apps/matrix-frontend/apps/matrix-frontend`, a non-existent path. Any future use of the alias will fail to resolve, increasing the risk of blank screens once the alias is referenced.

## Architecture map (high level)
- **Frontend (`apps/matrix-frontend`)**: Vite/React single-page app bootstrapped from `index.tsx` → `App.tsx` → `WidgeTDC_Pro.jsx`, which orchestrates dynamic widget loading and grid layout management.
