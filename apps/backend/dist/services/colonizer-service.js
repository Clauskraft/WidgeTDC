...
export class ColonizerService {
    constructor() {
        this.toolsDir = path.resolve(__dirname, '../tools/generated');
     ...
     */
    async assimilateSystem(options) {
     ...
     */
    async analyzeApi(systemName, spec) {
        ...
        const endpointPatterns = lines.filter(l => /\b(GET|POST|PUT|DELETE|PATCH)\s+\//.test(l));
        return {
     ...
     */
    buildAssimilationPrompt(systemName, apiSpec, analysis) {
...
Output KUN koden indkapslet i \`\`\`typescript ... \`\`\` blokke.
     ...
     */
    validateGeneratedCode(code, systemName) {
        const warnings = [];
     ...
     */
    async generateTests(systemName, code) {
        const messages = [
            {
                role: 'system',
...
Output kun koden.`
     ...
     */
    async logAssimilation(systemName, endpointCount) {
        try {
            const { hyperLog } = await import('./hyper-log.js');
            await hyperLog.log('INSIGHT', 'TheColonizer', `Nyt system assimileret: ${systemName} (${endpointCount} endpoints)`, {
                systemName,
                endpointCount,
