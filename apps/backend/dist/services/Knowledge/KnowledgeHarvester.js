...
export class KnowledgeHarvester {
    constructor() {
        ...
        this.libraryDir = path.resolve(__dirname, '../../../../packages/knowledge/library');
     ...
     */
    async harvestFromUrl(url, filename, category) {
        try {
            console.log(`[HARVESTER] 🚜 Starter høst fra: ${url}`);
            const response = await fetch(url);
            if (!response.ok)
                throw new Error(`Kunne ikke hente URL: ${response.statusText} (${response.status})`);
