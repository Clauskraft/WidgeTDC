 ...
 */
export function initAutonomousAgent() {
    if (agent)
        return agent;
...
autonomousRouter.post('/manage_project_memory', async (req, res) => {
    try {
        ...
        const { eventType, event_type, component_name, status, details, metadata, name, description, featureStatus, limit } = params;
...
autonomousRouter.post('/graphrag', async (req, res) => {
    try {
        ...
        const userId = req.user?.id || context?.userId || 'anonymous';
        const orgId = req.user?.orgId || context?.orgId || 'default';
        if (!query) {
            return res.status(400).json({ error: 'Query is required' });
        }
...
autonomousRouter.post('/agentteam', async (req, res) => {
    try {
        ...
        const userId = req.user?.id || metadata?.userId || 'anonymous';
        const orgId = req.user?.orgId || metadata?.orgId || 'default';
        if (!content) {
...
autonomousRouter.get('/agentteam/pal/history', async (req, res) => {
    try {
        ...
        const history = palAgent.getConversationHistory?.() || [];
        res.json({
            success: true,
            history,
...
autonomousRouter.get('/health', async (req, res) => {
    try {
        const registry = getSourceRegistry();
        const sources = registry.getAllSources();
        const sourceHealth = await Promise.all(sources.map(async (source) => {
            try {
