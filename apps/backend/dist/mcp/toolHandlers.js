...
export async function cmaContextHandler(payload, _ctx) {
    ...
    const memoryContext = memories.map(m => `[${m.entity_type}] ${m.content} (importance: ${m.importance})`).join('\n');
...
${payload.widgetData || 'None'}
  `.trim();
    ...
    const aiResponse = await llmService.generateContextualResponse(systemContext, payload.userQuery, additionalContext, payload.model);
    return {
        response: aiResponse,
...
export async function cmaMemoryStoreHandler(payload, ctx) {
...
// SRAG tool handlers
// SRAG tool handlers
export async function sragQueryHandler(payload, ctx) {
...
`;
        if (isAnalytical) {
            ...
            const aiResponse = await llmService.generateContextualResponse(systemContext, rawQuery, factsContext, model);
            ...
            const aiResponse = await llmService.generateContextualResponse(systemContext, rawQuery, docsContext, model);
            console.log('✅ LLM response received');
            return {
                type: 'semantic',
                response: aiResponse,
...
export async function evolutionReportHandler(payload, _ctx) {
...
Recent Run: ${JSON.stringify(payload, null, 2)}
  `.trim();
    const aiAnalysis = await llmService.generateContextualResponse(systemContext, 'Analyze this agent performance and provide recommendations', metricsContext, payload.model);
    return {
        runId,
...
export async function evolutionGetPromptHandler(payload, _ctx) {
    const { agentId } = payload;
    const prompt = evolutionRepo.getLatestPrompt(agentId);
    return { prompt };
}
export async function evolutionAnalyzePromptsHandler(payload, _ctx) {
    const { agentId } = payload;
    ...
    const aiAnalysis = await llmService.generateContextualResponse(systemContext, 'Analyze prompt evolution and suggest improvements', promptsContext, payload.model);
    return {
        prompts,
        aiAnalysis,
    };
...
export async function palAnalyzeSentimentHandler(payload, ctx) {
    const { text } = payload;
...
// Notes tool handlers
export async function notesListHandler(payload, ctx) {
    const notes = notesRepo.getNotes(ctx.userId, ctx.orgId);
    return { notes };
}
export async function notesCreateHandler(payload, ctx) {
...
export async function notesUpdateHandler(payload, ctx) {
...
export async function autonomousEvolutionHandler(payload, _ctx) {
...
export async function autonomousAgentTeamHandler(payload, _ctx) {
    const { task, context } = payload;
    // Use coordinate method to handle tasks
    const result = await agentTeam.coordinate(task, context);
    return result;
}
export async function autonomousAgentTeamCoordinateHandler(payload, _ctx) {
    const { task, context } = payload;
    const result = await agentTeam.coordinate(task, context);
    return result;
}
// Vidensarkiv (PgVector) tool handlers
export async function vidensarkivSearchHandler(payload, ctx) {
...
export async function vidensarkivAddHandler(payload, ctx) {
    const vectorStore = getNeo4jVectorStore();
...
export async function vidensarkivBatchAddHandler(payload, ctx) {
    const vectorStore = getNeo4jVectorStore();
    const { records, namespace } = payload;
    const vectorRecords = records.map((r, idx) => ({
...
export async function vidensarkivGetRelatedHandler(payload, ctx) {
    const vectorStore = getNeo4jVectorStore();
...
export async function vidensarkivListHandler(payload, _ctx) {
    const vectorStore = getNeo4jVectorStore();
...
export async function vidensarkivStatsHandler(_payload, _ctx) {
    const vectorStore = getNeo4jVectorStore();
    const stats = await vectorStore.getStatistics();
    return {
        success: true,
 ...
 */
export async function taskRecorderGetSuggestionsHandler(_payload, _ctx) {
 ...
 */
export async function taskRecorderApproveHandler(payload, ctx) {
 ...
 */
export async function taskRecorderRejectHandler(payload, ctx) {
    const { suggestionId } = payload;
    if (!suggestionId) {
        throw new Error('suggestionId is required');
    }
 ...
 */
export async function taskRecorderExecuteHandler(payload, ctx) {
 ...
 */
export async function taskRecorderGetPatternsHandler(_payload, _ctx) {
    const recorder = getTaskRecorder();
 ...
 */
export async function widgetsInvokeHandler(payload, ctx) {
 ...
 */
export async function widgetsOsintInvestigateHandler(payload, ctx) {
 ...
 */
export async function widgetsThreatHuntHandler(payload, ctx) {
 ...
 */
export async function widgetsOrchestratorCoordinateHandler(payload, ctx) {
    ...
    const coordinationResult = await agentTeam.coordinate(`investigate:${type}:${target}`, { target, type, phases, userId: ctx.userId });
 ...
 */
export async function docgenPowerpointCreateHandler(payload, ctx) {
 ...
 */
export async function docgenWordCreateHandler(payload, ctx) {
    const { title, type = 'report', topic, targetWordCount = 3000, includeExecutiveSummary = true, tone = 'professional' } = payload;
    if (!title || !topic) {
        throw new Error('title and topic are required');
    }
 ...
 */
export async function docgenExcelCreateHandler(payload, ctx) {
    const { title, analysisType = 'financial', dataSource, includeCharts = true, includeFormulas = true, includeDashboard = true } = payload;
    if (!title) {
 ...
 */
export async function docgenStatusHandler(payload, _ctx) {
...
// ---------------------------------------------------
export async function emailRagHandler(payload, ctx) {
...
// ---------------------------------------------------
export async function agenticRunHandler(payload, ctx) {
    ...
    const firstTool = payload.workflow.nodes.find((n) => n.type === 'tool');
    if (!firstTool) {
...
// ---------------------------------------------------
export async function widgetsUpdateStateHandler(payload, ctx) {
...
// ---------------------------------------------------
export async function widgetsAudioTranscribeHandler(payload, ctx) {
...
// ---------------------------------------------------
export async function widgetsImageAnalyzeHandler(payload, ctx) {
