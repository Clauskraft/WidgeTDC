...
export class PatternMemory {
    ...
    async getWidgetPatterns(widgetId) {
      ...
      `, [widgetId]);
            ...
            for (const p of patterns) {
                sourceCounts.set(p.source_used, (sourceCounts.get(p.source_used) || 0) + p.count);
                const hour = parseInt(p.hour || '0');
                timePatterns.set(hour, (timePatterns.get(hour) || 0) + p.count);
    ...
    async insertPattern(pattern) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
            stmt.run(pattern.id, pattern.widgetId, pattern.queryType, pattern.querySignature, pattern.sourceUsed, pattern.latencyMs, pattern.resultSize || null, pattern.success ? 1 : 0, JSON.stringify(pattern.userContext), pattern.timestamp.toISOString());
        }
        else {
            // SQL.js (async)
            this.db.run(`
    ...
    async queryPatterns(sql, params) {
        if (this.db.prepare) {
            // better-sqlite3
