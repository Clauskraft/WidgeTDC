...
export class FailureMemory {
     ...
     */
    async getFailureHistory(sourceName, limit = 50) {
    ...
    async getRecoveryPaths(sourceName, errorType) {
        const matching = this.failures.filter(f => f.sourceName === sourceName &&
    ...
    async getLastSuccessfulRecovery(sourceName, errorType) {
        const lastSuccess = this.failures.find(f => f.sourceName === sourceName &&
            f.errorType === errorType &&
            f.recoverySuccess &&
            f.recoveryAction);
        if (!lastSuccess)
     ...
     */
    async isRecurringFailure(sourceName, errorType, withinMinutes = 60) {
        const cutoff = new Date();
        cutoff.setMinutes(cutoff.getMinutes() - withinMinutes);
        const recentCount = this.failures.filter(f => f.sourceName === sourceName &&
            f.errorType === errorType &&
            f.occurredAt > cutoff).length;
    ...
    async getFailureStats(sourceName) {
        ...
        const recentFailures = this.failures.filter(f => f.sourceName === sourceName && f.occurredAt > sevenDaysAgo);
