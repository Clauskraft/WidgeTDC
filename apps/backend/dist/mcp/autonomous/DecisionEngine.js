...
export class DecisionEngine {
     ...
     */
    async scoreAllSources(sources, intent) {
        const scores = await Promise.all(sources.map(source => this.scoreSource(source, intent)));
     ...
     */
    async scoreSource(source, intent) {
        ...
        // Weighted total
        const totalScore = performance * weights.performance +
     ...
     */
    async scorePerformance(source, intent) {
     ...
     */
    async scoreReliability(source, intent) {
        ...
        const successRate = await this.memory.getSuccessRate(source.name, intent.type);
     ...
     */
    async scoreHistory(source, intent) {
        // Check if this source has been successful for similar queries
        const historyScore = await this.memory.getSimilarQuerySuccess(intent.type, intent.params);
        return historyScore;
    }
    /**
     ...
     */
    generateReasoning(breakdown, weights) {
        const reasons = [];
        ...
        for (const [factor, score] of factors) {
            if (score < 0.3 && weights[factor] > 0.15) {
    ...
    inferDomain(query) {
        // Simple heuristics
        if (query.uri?.startsWith('agents://'))
            return 'agents';
        if (query.uri?.startsWith('security://'))
            return 'security';
        if (query.tool?.includes('search'))
            return 'search';
        if (query.tool?.includes('agent'))
            return 'agents';
        return 'general';
    }
}
