...
export class AgentOrchestratorServer {
    constructor() {
        ...
        this.version = '1.1.0';
        this.registryPath = join(process.cwd(), '../../agents/registry.yml');
    ...
    async checkAndTriggerDependents(completedAgentId) {
        ...
        for (const [id, agent] of this.agentStatuses.entries()) {
            if (agent.status !== 'idle')
                continue; // Only trigger idle agents
    ...
    async triggerAgent(agentId) {
        ...
        if (workflowFile) {
            ...
            try {
                ...
                exec(`gh workflow run ${workflowFile}`, (error, stdout, stderr) => {
                    if (error) {
