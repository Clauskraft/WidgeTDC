import { agentCommunicationProtocol } from './AgentCommunication';
/**
...
export class AgentSpawner {
     ...
     */
    findBestAgent(requiredCapabilities) {
        const candidates = Array.from(this.agents.values()).filter(agent => {
            // Check if agent has required capabilities
            const hasCapabilities = requiredCapabilities.every(required => agent.capabilities.some(cap => cap.name === required));
...
export class AgentSpecializationLearner {
     ...
     */
    recordPerformance(agentId, task, success, duration) {
     ...
     */
    identifySpecializations(agentId) {
        const history = this.performanceHistory.get(agentId) || [];
...
export class KnowledgeSharing {
     ...
     * Share knowledge
     */
    async shareKnowledge(fromAgent, topic, knowledge) {
     ...
     */
    rateKnowledge(topic, knowledgeIndex, rating) {
        const knowledge = this.sharedKnowledge.get(topic);
