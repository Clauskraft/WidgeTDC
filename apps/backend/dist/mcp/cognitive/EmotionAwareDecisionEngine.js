...
export class EmotionAwareDecisionEngine {
    ...
    async makeDecision(query, ctx) {
        ...
        const queryIntent = typeof query === 'string'
            ? {
                type: 'query',
                domain: 'general',
                operation: 'search',
     ...
     */
    async getEmotionalState(userId, orgId) {
        try {
            ...
            if (Array.isArray(recentEvents)) {
                const stressEvents = recentEvents.filter((e) => e.event_type === 'stress' || e.detected_stress_level);
                if (stressEvents.length > 0) {
                    const avgStress = stressEvents.reduce((sum, e) => {
                ...
                const inFocusWindow = focusWindows.some((fw) => fw.weekday === currentDay &&
     ...
     */
    async evaluateDataQuality(query, _ctx) {
        try {
     ...
     */
    evaluateEmotionalFit(action, emotion) {
     ...
     */
    async evaluateContextRelevance(query, ctx) {
        try {
     ...
     * Calculate dynamic weights based on emotional state
     */
    calculateDynamicWeights(emotion) {
        // Base weights
        let dataWeight = 0.4;
     ...
     * Fuse scores with weights
     */
    fusionDecision(scores, weights) {
        return (scores.data * weights.data +
     ...
     */
    determineOptimalAction(query, emotion) {
     ...
     */
    generateReasoning(emotion, dataScore, emotionScore, contextScore) {
        const parts = [];
        if (emotion.stress === 'high') {
