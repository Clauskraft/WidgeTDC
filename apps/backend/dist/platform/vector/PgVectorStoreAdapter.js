...
export class PgVectorStoreAdapter {
        ...
        if (this.useSQLite) {
            ...
            await prisma.$executeRawUnsafe(`
                ...
                DO UPDATE SET 
                    ...
                    "updatedAt" = NOW()
            `, record.id, record.content, embeddingStr, JSON.stringify(record.metadata || {}), record.namespace || 'default', record.metadata?.userId || 'system', record.metadata?.orgId || 'default');
        }
    }
    /**
    ...
    async getStatistics() {
        ...
        if (this.useSQLite) {
            ...
            const namespaceCounts = await prisma.$queryRaw `
