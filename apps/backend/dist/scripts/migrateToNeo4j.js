...
export async function migrateMemoryToNeo4j() {
    ...
    try {
        ...
        const entities = db.prepare('SELECT * FROM memory_entities').all();
        const entityIdMap = new Map(); // SQLite ID -> Neo4j ID
        for (const entity of entities) {
            const neo4jNode = await graphMemoryService.createEntity(entity.entity_type, entity.content.substring(0, 100), // Use first 100 chars as name
            {
                orgId: entity.org_id,
            ...
            // Migrate tags
            const tags = db.prepare('SELECT tag FROM memory_tags WHERE entity_id = ?').all(entity.id);
            for (const tagRow of tags) {
                // Add tag as property or create separate Tag nodes
                await neo4jService.runQuery(`MATCH (n) WHERE id(n) = $id 
           SET n.tags = CASE WHEN n.tags IS NULL THEN [$tag] ELSE n.tags + $tag END`, { id: parseInt(neo4jNode.id), tag: tagRow.tag });
            }
        }
        ...
        for (const relation of relations) {
            ...
            if (sourceNeo4jId && targetNeo4jId) {
                await graphMemoryService.createRelation(sourceNeo4jId, targetNeo4jId, relation.relation_type, {
