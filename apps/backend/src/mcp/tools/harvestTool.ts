...
import { harvesterService, HarvestResult } from '../../services/harvester/HarvesterService.js';
 ...
 */
export async function harvestFetchHandler(
  payload: { url: string; filename?: string; category?: string },
  ctx: McpContext
  ...
  await hyperLog.log('USER_INTENT', 'TheArchitect',
    `Commanded to harvest: ${url}${category ? ` -> library/${category}` : ''}`, {
      url,
      filename,
      category,
  ...
  if (result.success) {
    await hyperLog.log('INSIGHT', 'TheArchitect',
      `New knowledge acquired: ${result.category ? `library/${result.category}/` : ''}${result.filename}`, {
        source: url,
        size: result.size,
 ...
 */
export async function harvestListHandler(
  payload: { category?: string },
  _ctx: McpContext
): Promise<{ success: boolean; files: string[]; count: number; location: string; libraryPath: string }> {
 ...
 */
export async function harvestReadHandler(
  payload: { filename: string; category?: string },
  _ctx: McpContext
): Promise<{
  success: boolean;
  content?: string;
