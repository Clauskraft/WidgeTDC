  ...
  try {
    ...
    const personaFeedbackMap: Record<string, PersonaFeedback> = {
      ...
      backend: {
        ...
        role: 'Backend Expert',
        feedback:
          'API-designet følger RESTful principper godt. Performance-overvejelser skal dog specificeres mere detaljeret.',
        confidence: 0.88,
        recommendations: [
      ...
      frontend: {
        ...
        feedback:
          'UI/UX-beskrivelsen er god, men der mangler detaljer om accessibility og responsive design implementering.',
        confidence: 0.79,
        recommendations: [
    ...
    const personas: PersonaFeedback[] = selectedPersonas.filter((p) => personaFeedbackMap[p]).map((p) => personaFeedbackMap[p]);
    ...
    const result: ReviewResult = {
      summary:
        `${personas.length} eksperter har gennemgået specifikationen. ` +
        'Overordnet vurdering: God struktur med behov for flere tekniske detaljer.',
      consensus,
      disagreements,
      personas,
      overallScore,
