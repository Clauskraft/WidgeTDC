...
export const MSWidgetRenderer: React.FC<MSWidgetRendererProps> = ({ widget }) => {
  return (
      ...
      <div className="p-4 flex-1 overflow-y-auto space-y-4 text-sm">
        ...
        {widget.dataSource?.url && (
          <InfoItem label="Data Source">
            <a
              href={widget.dataSource.url}
              target="_blank"
              rel="noopener noreferrer"
              className="ms-focusable text-blue-500 hover:underline break-all"
            >
              {widget.dataSource.url}
            </a>
          </InfoItem>
        ...
        <InfoItem label="Capabilities">
          <div className="flex flex-wrap gap-2">
            {widget.capabilities.length > 0 ? (
              widget.capabilities.map(cap => (
                <span
                  key={cap}
                  className="px-2 py-1 text-xs font-medium rounded-full bg-gray-200 text-gray-700 dark:bg-gray-600 dark:text-gray-200 capitalize"
                >
                  {cap}
                </span>
              ))
