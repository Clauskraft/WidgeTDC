...
const PerformanceMonitorWidget: React.FC<{ widgetId: string }> = () => {
  ...
  useEffect(() => {
    function connect() {
      ...
      socketRef.current.onclose = (event: CloseEvent) => {
        ...
        if (!event.wasClean) {
          console.error(
            `Performance Monitor WebSocket closed unexpectedly. Code: ${event.code}, Reason: "${event.reason}"`
          );
        }
      ...
      socketRef.current.onerror = (error: Event) => {
        console.error(
          'Performance Monitor WebSocket Error. See the event object for details:',
          error
        );
  ...
  const MetricDisplay: React.FC<{ label: string; value: number; unit: string }> = ({
    label,
    value,
    unit,
  }) => (
