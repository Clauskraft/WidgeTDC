...
const WidgetImporterWidget: React.FC<{ widgetId: string }> = () => {
  ...
  const handleDirectImport = (msWidget: MSWidget, onSuccess: (message: string) => void) => {
    ...
    onSuccess(
      `Widget'en "${newWidgetDefinition.name}" er blevet tilføjet til sidebaren og er klar til brug.`
    );
  };
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      setScanError(
        'Kunne ikke scanne efter widgets. Denne funktion er muligvis kun tilgængelig i en desktop-app.'
      );
    } finally {
      setIsScanning(false);
    }
  ...
  const renderManualImporter = () => (
    <>
      <div className="flex flex-col gap-2 flex-1">
        <label htmlFor="ms-widget-json" className="font-semibold">
          Indsæt Microsoft Widget JSON
        </label>
        <textarea
          id="ms-widget-json"
          value={inputJson}
      ...
      {(importError || importSuccess) && (
        <div className="pt-2">
          {importError && (
            <div className="p-3 text-sm rounded-lg bg-red-50 dark:bg-red-900/50 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-200">
              {importError}
            </div>
          )}
          {importSuccess && (
