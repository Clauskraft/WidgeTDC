...
const ApiIcon = (props: { className?: string }) => <svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>;
const FileIcon = (props: { className?: string }) => <svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>;
const StatusIndicator: React.FC<{ status: ConnectionStatus }> = ({ status }) => {
    ...
    return <div className="flex items-center gap-2 text-xs font-medium"><div className={`w-2.5 h-2.5 rounded-full ${statusMap[status].color}`} /><span>{statusMap[status].text}</span></div>;
};

// --- Data Definitions ---
const DATA_SOURCES: DataSource[] = [
    { id: 'postgresql', name: 'PostgreSQL', category: 'Database', icon: DatabaseIcon, fields: [
        { name: 'host', label: 'Host', type: 'text', placeholder: 'f.eks. localhost', required: true },
        ...
        { name: 'password', label: 'Adgangskode', type: 'password', placeholder: '••••••••', required: true },
    ]},
    { id: 'rest_api', name: 'REST API', category: 'API', icon: ApiIcon, fields: [
        { name: 'base_url', label: 'Base URL', type: 'text', placeholder: 'https://api.example.com', required: true },
        { name: 'api_key', label: 'API Nøgle', type: 'password', placeholder: 'Indsæt din API nøgle', required: false },
    ]},
    { id: 'csv_upload', name: 'CSV Fil', category: 'File', icon: FileIcon, fields: [
        { name: 'file_path', label: 'Filsti', type: 'text', placeholder: '/path/to/your/file.csv', required: true },
    ]},
];
...
const MCPConnectorWidget: React.FC<{ widgetId: string }> = () => {
    ...
    const renderListView = () => (
        <div className="space-y-3">
            ...
            {connections.length > 0 ? connections.map(conn => {
                ...
                return (
                    <div key={conn.id} className="p-3 flex items-center justify-between rounded-lg bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700">
    ...
    const renderConfigureView = () => {
        ...
        const testBtnText = { idle: 'Test Forbindelse', testing: 'Tester...', success: 'Succes!', error: 'Fejl - Prøv igen' };
