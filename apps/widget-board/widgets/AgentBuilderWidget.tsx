...
const AgentBuilderWidget: React.FC<{ widgetId: string }> = () => {
  ...
  const [tools, setTools] = useState<ToolSuggestion[]>(
    ALL_TOOLS.map(t => ({ ...t, status: 'optional' }))
  );
  ...
  useEffect(() => {
    async function fetchSuggestions() {
      ...
      try {
        ...
        const mockData = {
          tools: [
            debouncedInstruction.includes('beregn') && {
              name: 'calculator',
              status: 'recommended' as const,
            },
            debouncedInstruction.includes('søg') && {
              name: 'public_search',
              status: 'recommended' as const,
            },
            debouncedInstruction.includes('privat') && {
              name: 'private_search',
      ...
      <div className="flex-1 p-4 space-y-4 overflow-y-auto">
        <div className="border-b border-gray-200 dark:border-gray-700 pb-4 mb-4">
          ...
          >
            ...
            <button
              className="ms-focusable p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700"
              aria-label={isListVisible ? 'Skjul liste' : 'Vis liste'}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className={`h-5 w-5 transition-transform duration-200 ${isListVisible ? 'rotate-180' : ''}`}
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
          ...
          {isListVisible && (
            <div
              id="created-agents-list"
              className="mt-2 max-h-48 overflow-y-auto pr-2 -mr-2 space-y-3"
            >
              {createdAgents.length > 0 ? (
                createdAgents.map(agent => (
                  <div
                    key={agent.id}
                    className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col gap-2"
                  >
                    <div className="flex items-center gap-2">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <strong className="text-xs font-semibold text-gray-500 dark:text-gray-400">
                        Værktøjer:
                      </strong>
                      {agent.tools.length > 0 ? (
                        agent.tools.map(tool => (
                          <span
                            key={tool}
                            className="px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200 capitalize"
                          >
                            {tool.replace('_', ' ')}
                          </span>
                        ))
        ...
        <div>
          <label htmlFor="agentName" className="text-sm font-medium mb-1 block">
            Agent Navn
          </label>
          <input
            id="agentName"
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Beskriv agentens opgaver og kompetenceområde..."
            rows={4}
            className="ms-focusable w-full p-2 rounded-lg border bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600 resize-none"
          />
        </div>
            ...
            {tools.map(tool => (
              ...
              >
                ...
                <div className="mt-auto pt-1">
                  <span
                    className={`text-xs px-2 py-1 rounded-full font-semibold ${getStatusBadgeClasses(tool.status)}`}
                  >
